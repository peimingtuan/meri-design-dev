...
const TimeDrop = (options) => {
    ...
    const { X, Y, P } = CalcTargetPosition(tag, dom, 372, 624);
    param.position = P;
    vm = new TimeDropExtend({ data: param, propsData: props }); // 实例化
...
TimeDrop.resetPosition = (vm) => {
    const { X, Y, P } = CalcTargetPosition(vm.tag, vm.dom, 372, 624);
    vm.position = P;
    vm.$el.style.top = `${Y}px`;
    vm.$el.style.left = `${X}px`;
};
