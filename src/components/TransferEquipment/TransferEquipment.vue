        ...
        <div class="p-transfer-main">
            ...
            <div class="p-transfer-main-child p-transfer-right">
                <section :class="['p-transfer-right-title', 'p-transfer-right-title-border']">
                    ...
                    >已选设备<span class="p-transfer-right-title-text-num" v-show="selectedData&&selectedData.length"
                    >{{selectedData.length}}</span></article>
                    <article :class="['p-transfer-right-clear', (selectedData&&selectedData.length)&&'p-transfer-right-clear-active']" @click="handleEmpty"
                    >清空</article>
                </section>
                <section class="p-transfer-selected">
                    <article class="p-transfer-selected-item" v-for="(sd, i) in selectedData" :key="'sd-'+sd.id">
                        <span @mouseenter="itemEnter">
                            {{sd.name}}
                            <Tag v-if="sd.tag" size='small' :color="sd.tag.color">{{sd.tag.name}}</Tag>
                        </span>
...
import {
    TileTool, FilterTool, ChangeStatus, GetParentIdById, Unique, PackageTool, initTreeData
...
export default {
    ...
    computed: {
        ...
        */
        ...
        mulData: {
            get() {
                const treeData = initTreeData(PackageTool(TileTool([], this.treeData, '-1')));
                return treeData;
            },
            set(newData) {
                return newData;
    ...
    methods: {
        ...
        treeChange({
            ...
            if (linkage) {
                 ...
                 */
                const { checked, name, tag } = obj;
                if (checked === 'checked') {
                    this.selectedData.push({ id, name, tag });
                } else {
        ...
        setSelectedData(data, tree) {
            tree.forEach(d => {
                if (this.notNull) {
                    if ((d.checked === 'checked' || d.checked === 'notNull') && !d.disabled) data.push({ id: d.id, name: d.name, tag: d.tag });
                } else {
                    if (d.checked === 'checked' && !d.disabled) data.push({ id: d.id, name: d.name, tag: d.tag });
                }
