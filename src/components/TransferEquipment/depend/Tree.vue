<template>
    <div class="p-transfer-equipment-tree">
        <TreeNode
                :multiple="multiple"
                :linkage="linkage"
                :lastStage="lastStage"
...
export default {
    ...
    methods: {
         ...
         * @param linkage
         */
        change(obj, index, linkage = this.linkage) {
            const { id }=obj;
            if (this.multiple) {
                let iArr=(index.split('-')); // 拿到索引值
                iArr.pop(); // 这里不需要遍历最后一个索引的数据
                let data=this.treeData;
                // if (linkage) this.changeParentChecked(data, iArr);
                 ...
                 * @type Function
                 */
                this.$emit('change', {id, checkedIds, obj, checkedObj, linkage});
            } else {
        ...
        recursionIds(data, arr) {
            data.forEach(d => {
                if (this.notNull) {
                    if ((d.checked === 'checked' || d.checked === 'notNull') && !d.disabled) arr.push(d);
                } else {
                    if (d.checked === 'checked' && !d.disabled) arr.push(d);
                }
