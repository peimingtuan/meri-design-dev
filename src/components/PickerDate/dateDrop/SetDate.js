import CountPrevMonth from '../../static/utils/datePicker/CountPrevMonth';
import CountPrevYear from '../../static/utils/datePicker/CountPrevYear';

import { DateFormat, DateTimeFormat } from './DateFormatUtils';
import SetDateByTabKey from './SetDateByTabKey';

/**
 * 设置年月日
 * @param thisObj DateDrop的this
 * @param date 传入的日期
 * @param toggleVal 时间段
 * @param yearNow 当前年
 * @param monthNow 当前月
 * @param dayNow 当前日
 * @constructor
 */
const SetDate = (thisObj, date, toggleVal, yearNow, monthNow, dayNow) => {
    const { tabKey, format, timeStart } = thisObj;
    let DATE = date;
    thisObj.selectedDate = date;
    if (toggleVal) {
        // 多选
        if (date) {
            const [start, end] = DateFormat(date);
            thisObj.selectedDateStart = SetDateByTabKey(start, tabKey);
            thisObj.selectedDateEnd = SetDateByTabKey(end, tabKey);
            thisObj.clickSelectedDateStart = SetDateByTabKey(start, tabKey);
            thisObj.clickSelectedDateEnd = SetDateByTabKey(end, tabKey);
            const [dateS, timeS] = DateTimeFormat(start),
                [dateE, timeE] = DateTimeFormat(end);
            thisObj.ymdSelectedStart = dateS;
            thisObj.ymdSelectedEnd = dateE;
            const [sh = '00', sm = '00', ss = '00'] = timeS.split(':'),
                [eh = '00', em = '00', es = '00'] = timeE.split(':');
            thisObj.hourStart = sh;
            thisObj.minuteStart = sm;
            thisObj.secondStart = ss;
            thisObj.hourEnd = eh;
            thisObj.minuteEnd = em;
            thisObj.secondEnd = es;
            thisObj.timeStart = timeS;
            thisObj.timeEnd = timeE;
            const [ys, ms, ds] = dateS.split('.');
            let yas = ys,
                mas = ms,
                das = ds;
            const [ye, me, de] = dateE.split('.');
            const strategy = {
                day() {
                    if (ys + ms === ye + me) {
                        const [y, m, d] = CountPrevMonth([ys, ms, ds]).split('.');
                        yas = y;
                        mas = m;
                        das = d;
                    }
                },
                week() {
                    this.day();
                },
                month() {
                    if (ys === ye) {
                        const [y, m, d] = CountPrevYear([ys, ms, ds]).split('.');
                        yas = y;
                        mas = m;
                        das = d;
                    }
                },
                year() {
                    if (ye - ys < 12) {
                        yas = String(ye - 12);
                    }
                }
            };
            strategy[tabKey]();
            thisObj.yearSelectedStart = ys;
            thisObj.monthSelectedStart = ms;
            thisObj.daySelectedStart = ds;
            thisObj.yearActiveStart = yas;
            thisObj.monthActiveStart = mas;
            thisObj.dayActiveStart = das;
            thisObj.clickYearSelectedStart = ys;
            thisObj.clickMonthSelectedStart = ms;
            thisObj.clickDaySelectedStart = ds;
            thisObj.yearSelectedEnd = ye;
            thisObj.monthSelectedEnd = me;
            thisObj.daySelectedEnd = de;
            thisObj.yearActiveEnd = ye;
            thisObj.monthActiveEnd = me;
            thisObj.dayActiveEnd = de;
            thisObj.clickYearSelectedEnd = ye;
            thisObj.clickMonthSelectedEnd = me;
            thisObj.clickDaySelectedEnd = de;

            switch (tabKey) {
            case 'day':
                DATE = `${yas}.${mas}.${das}${(format && timeS) ? ` ${timeS}` : ''}-${ye}.${me}.${de}${(format && timeE) ? ` ${timeE}` : ''}`;
                break;
            case 'week':
                DATE = `${yas}.${mas}.${das}-${ye}.${me}.${de}`;
                break;
            case 'month':
                DATE = `${yas}.${mas}-${ye}.${me}`;
                break;
            case 'year':
                DATE = `${yas}-${ye}`;
                break;
            default:
                break;
            }
        } else {
            thisObj.selectedDateStart = '';
            thisObj.selectedDateEnd = '';
            thisObj.clickSelectedDateStart = '';
            thisObj.clickSelectedDateEnd = '';
            thisObj.ymdSelectedStart = '';
            thisObj.ymdSelectedEnd = '';
            thisObj.hourStart = '';
            thisObj.minuteStart = '';
            thisObj.secondStart = '';
            thisObj.hourEnd = '';
            thisObj.minuteEnd = '';
            thisObj.secondEnd = '';
            thisObj.timeStart = '';
            thisObj.timeEnd = '';
            thisObj.yearSelectedStart = '';
            thisObj.monthSelectedStart = '';
            thisObj.daySelectedStart = '';
            thisObj.clickYearSelectedStart = '';
            thisObj.clickMonthSelectedStart = '';
            thisObj.clickDaySelectedStart = '';
            thisObj.yearSelectedEnd = '';
            thisObj.monthSelectedEnd = '';
            thisObj.daySelectedEnd = '';
            thisObj.clickYearSelectedEnd = '';
            thisObj.clickMonthSelectedEnd = '';
            thisObj.clickDaySelectedEnd = '';

            thisObj.yearActiveEnd = yearNow;
            thisObj.monthActiveEnd = monthNow;
            thisObj.dayActiveEnd = dayNow;

            const [y, m, d] = CountPrevMonth([yearNow, monthNow, dayNow]).split('.');
            thisObj.yearActiveStart = y;
            thisObj.monthActiveStart = m;
            thisObj.dayActiveStart = d;

            // DATE = `${y}.${m}.${d}-${yearNow}.${monthNow}.${dayNow}`;
            if (tabKey === 'month' || tabKey === 'year') {
                const [py, pm, pd] = CountPrevYear([yearNow, monthNow, dayNow]).split('.');
                thisObj.yearActiveStart = py;
                thisObj.monthActiveStart = pm;
                thisObj.dayActiveStart = pd;
                // DATE = (tabKey === 'month') ? (`${py}.${pm}.${pd}-${yearNow}.${monthNow}.${dayNow}`) : (`${py}-${yearNow}`);
            }

            const { yearActiveStart, monthActiveStart, dayActiveStart } = thisObj;
            switch (tabKey) {
            case 'day':
            case 'week':
                DATE = `${yearActiveStart}.${monthActiveStart}.${dayActiveStart}-${yearNow}.${monthNow}.${dayNow}`;
                break;
            case 'month':
                DATE = `${yearActiveStart}.${monthActiveStart}-${yearNow}.${monthNow}`;
                break;
            case 'year':
                DATE = `${yearActiveStart}-${yearNow}`;
                break;
            default:
                break;
            }
        }
    } else {
        // 单选
        if (date) {
            // thisObj.clickSelectedDateStart = date;
            thisObj.selectedDateStart = SetDateByTabKey(date, tabKey);
            thisObj.clickSelectedDateStart = SetDateByTabKey(date, tabKey);
            if (date.includes(':')) {
                const [de, t] = date.split(/\s+/);
                DATE = de;
                thisObj.timeStart = t;
                const [sh, sm, ss] = t.split(':');
                thisObj.hourStart = sh;
                thisObj.minuteStart = sm;
                thisObj.secondStart = ss;
            }
        }
        let y, m, d;
        if (DATE) {
            const D = DATE.includes('-') ? DATE.split('-')[0] : DATE,
                [y1, m1, d1] = D.split('.');
            y = y1;
            m = m1;
            d = d1;
            thisObj.yearSelectedStart = y;
            thisObj.monthSelectedStart = m;
            thisObj.daySelectedStart = d;
            thisObj.clickYearSelectedStart = y;
            thisObj.clickMonthSelectedStart = m;
            thisObj.clickDaySelectedStart = d;
            thisObj.ymdSelectedStart = DATE;
        } else {
            y = yearNow;
            m = monthNow;
            d = dayNow;
            thisObj.clickSelectedDateStart = '';
            thisObj.yearSelectedStart = '';
            thisObj.monthSelectedStart = '';
            thisObj.daySelectedStart = '';
            thisObj.clickYearSelectedStart = '';
            thisObj.clickMonthSelectedStart = '';
            thisObj.clickDaySelectedStart = '';
            thisObj.ymdSelectedStart = '';
        }
        thisObj.yearActiveStart = y;
        thisObj.monthActiveStart = m;
        thisObj.dayActiveStart = d;
        switch (tabKey) {
        case 'day':
            DATE = `${y}.${m}.${d}${(format && timeStart) ? ` ${timeStart}` : ''}`;
            break;
        case 'week':
            DATE = `${y}.${m}.${d}`;
            break;
        case 'month':
            DATE = `${y}.${m}`;
            break;
        case 'year':
            DATE = y;
            break;
        default:
            break;
        }
    }

    return DATE;
};

export default SetDate;
