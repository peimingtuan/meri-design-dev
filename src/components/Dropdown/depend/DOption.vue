<template>
    <transition :name="position?'selectDownUpExtend':'selectDownUpExtendTop'">
        <div v-show="optionStatus"
            ...
            >
            <label v-if="openSearch" class="p-drop-search" :class="{'p-drop-search-shadow': scrollTop > 12}">
            ...
            <div class="p-drop-option" ref="dropOption" @scroll="scrollTopHandle">
                <section
                    ...
                    :key="`${ind}-${item.id}`"
                    @click.stop="optionClick(item.id, item.name, item.disabled)"
