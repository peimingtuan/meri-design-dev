...
class CountWeek {
    ...
    static weekNumByMonth2(Y, M, D) {
        const nowDate = new Date(Y, Number(M) - 1, D); // 当前日期
        const firstDay = new Date(Y, Number(M) - 1, 1); // 当月第一天
        const dayOfWeek = firstDay.getDay(); // 当月第一天是星期几
        let spendDay; // 第二周第一天是几号
        if (dayOfWeek > 1) spendDay = 7 - dayOfWeek + 1 + 1;
        else if (dayOfWeek === 1) spendDay = 1;
        else spendDay = 2;
        const startDay = new Date(Y, Number(M) - 1, spendDay);
    ...
    static sliceArrayForMonth2(ls) {
        //摘出当月数据
        let newDay=[]
        ls.map(item => {
            if(item.flag==='p'){
                item.day=''
                item.month=Number(item.month)>8?`${Number(item.month)+1}`:`0${Number(item.month)+1}`
            }
            if(item.flag==='n')newDay.push(item)
        })
        const firstDay = new Date(newDay[0].year, Number(newDay[0].month) - 1, 1); // 当月第一天
        const dayOfWeek = firstDay.getDay(); // 当前月第一天是星期几
        let length=null;
        ...
        // const moreNum=(newDay.length-length)%7;
        // const dayNum=newDay.length-length-moreNum
        let result = [
            {
                // weeks:newDay.slice(0,length),
                weeks:ls.slice(0,7),
                selected: '',
                multiple: '',
                year:ls[0].year,
                month:ls[0].month,
                th:1,
                thText: `${newDay[0].year}.${newDay[0].month}第01周`,
                flag:'n' // 是否是其他月的周 p-其他月的周，n-当前月的周
            }
        ];
        const len = 7; // 长度
        for (let i = length; i < newDay.length; i += len) {
            const weeks = newDay.slice(i, i + len);
            const {
                year, month, day, flag
            } = weeks[0];
            const th = CountWeek.weekNumByMonth2(year, month, day);
    ...
    getWeeksArray() {
        ...
        const arr = this.sort === 'year'
            ...
            : this.sort === 'week'? CountWeek.sliceArrayForMonth2(res):CountWeek.sliceArrayForMonth(res);
